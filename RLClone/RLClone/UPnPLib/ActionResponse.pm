package Plugins::RLClone::UPnPLib::ActionResponse;

#-----------------------------------------------------------------
# Plugins::RLClone::UPnPLib::ActionResponse
#-----------------------------------------------------------------
...
use Plugins::RLClone::UPnPLib::HTTPResponse;
...
sub new {
	my($class) = shift;
	my($this) = {
		$Plugins::RLClone::UPnPLib::ActionResponse::_HTTP_RESPONSE => undef,
	};
	bless $this, $class;
}
...
sub sethttpresponse() {
	my($this) = shift;
	$this->{$Plugins::RLClone::UPnPLib::ActionResponse::_HTTP_RESPONSE} = $_[0];
 }

sub gethttpresponse() {
	my($this) = shift;
	$this->{$Plugins::RLClone::UPnPLib::ActionResponse::_HTTP_RESPONSE};
 }
...
sub getargumentlist() {
	...
	while ($soap_response =~ m/<([^>]*)>([^<]*)<\/[^>]*>/sg) {
		...
		$arg_value = $2;
		$arg_value = Plugins::RLClone::UPnPLib::HTTP::xmldecode($arg_value);
...
Plugins::RLClone::UPnPLib::ActionResponse - Perl extension for UPnP.
