package Plugins::RLClone::UPnPLib::AV::MediaServer;

#-----------------------------------------------------------------
# Plugins::RLClone::UPnPLib::AV::MediaServer
#-----------------------------------------------------------------
...
use Plugins::RLClone::UPnPLib::AV::Container;
use Plugins::RLClone::UPnPLib::AV::Item;
...
sub new {
	my($class) = shift;
	my($this) = {
		$Plugins::RLClone::UPnPLib::AV::MediaServer::_DEVICE => undef,
	};
	bless $this, $class;
}
...
sub setdevice() {
	my($this) = shift;
	if (@_) {
		$this->{$Plugins::RLClone::UPnPLib::AV::MediaServer::_DEVICE} = $_[0];
	}
}

sub getdevice() {
	my($this) = shift;
	$this->{$Plugins::RLClone::UPnPLib::AV::MediaServer::_DEVICE};
}
...
sub browse {
	...
	$dev = $this->getdevice();
	$condir_service = $dev->getservicebyname($Plugins::RLClone::UPnPLib::AV::MediaServer::CONTENTDIRECTORY_SERVICE_TYPE);
...
sub getcontentlist {
	...
	while ($result =~ m/<container(.*?)<\/container>/sgi) {
		$content = $1;
		$container = Plugins::RLClone::UPnPLib::AV::Container->new();
		if ($content =~ m/id=\"(.*?)\"/si) {
			$container->setid($1);
		}
	...
	while ($result =~ m/<item(.*?)<\/item>/sgi) {
		$content = $1;
		$item= Plugins::RLClone::UPnPLib::AV::Item->new();
		if ($content =~ m/id=\"(.*?)\"/si) {
			$item->setid($1);
		}
		if ($content =~ m/<dc:title>(.*)<\/dc:title>/si) {
		...
		if ($content =~ m/<res[^>]*>(.*?)<\/res>/si) {
			$item->seturl(Plugins::RLClone::UPnPLib::HTTP::xmldecode($1));
		}
...
Plugins::RLClone::UPnPLib::AV::MediaServer - Perl extension for UPnP.
...
=item B<new> - create new Plugins::RLClone::UPnPLib::AV::MediaServer.

    $mservier = Plugins::RLClone::UPnPLib::AV::MediaServer();
...
Browse the content directory and return the action response, L<Plugins::RLClone::UPnPLib::ActionResponse>.
...
Browse the content directory and return the content list. Please see L<Plugins::RLClone::UPnPLib::AV::Content>, L<Plugins::RLClone::UPnPLib::AV::Item> and L<Plugins::RLClone::UPnPLib::AV::Container>.
...
L<Plugins::RLClone::UPnPLib::AV::Container>
